...
app.post('/api/upload/finalize-link-item', ah(async (req, res) => {
  ...
  const item = await withDbWrite(async () => {
    ...
    const cleanTg = String(telegramUrl || '').trim();
    const dup = (db.links || []).find((l) =>
      (cleanUrl && l.url === cleanUrl) ||
      (cleanTg && (l.telegramUrl === cleanTg))
    );
...
app.get('/api/anymovie/result/:requestId', ah(async (req, res) => {
  ...
  // Once a card is saved for this search, prefer the card's own resolved
  // watchUrl (the /dl/ link generated by the link bot) when it's ready, so the
  // user lands on the real watching page instead of an un-resolved url.
  let cardWatchUrl = null;
  let cardResolved = false;
  if (r.savedItemId) {
    const card = (db.links || []).find((l) => l.id === r.savedItemId);
    if (card) {
      if (card.watchUrl) {
        cardWatchUrl = card.watchUrl;
        cardResolved = true;
      } else if (card.resolvedUrl) {
        cardWatchUrl = '/watch?url=' + encodeURIComponent(card.resolvedUrl)
          + '&title=' + encodeURIComponent(card.title || 'Watch');
        cardResolved = true;
      }
    }
  }
...
app.post('/api/anymovie/select-result', ah(async (req, res) => {
  ...
  await withDbWrite(async () => {
    ...
    if (r) {
      ...
      // file/link never appears twice.
      if (save && resultUrl) {
        const cardTitle = String(title || r.query || '🎬 Any Movie').trim();
        const cardUrl = String(resultUrl).trim();
        const cardTg = String(telegramUrl || '').trim();
        const cardThumb = String(thumbnailUrl || '').trim();

        const dup = (d.links || []).find((l) =>
          (cardUrl && l.url === cardUrl) ||
          (cardTg && (l.telegramUrl === cardTg))
        );

        if (dup) {
          savedItem = { ...dup, duplicate: true };
          // If the file was freshly forwarded (instantGet) and the card exists,
          // make sure a link-bot link is (re)generated for it.
          if (instantGet) {
            const iw = await ensureInstantGet(d, dup.id, dup.url || dup.telegramUrl, dup.title, dup.thumbnailUrl, dup.telegramUrl);
            if (iw) instantRequest = iw;
          }
        } else {
          const newItem = {
            id: makeId(),
          ...
          if (cardThumb) newItem.thumbnailUrl = cardThumb;
          d.links.unshift(newItem);
          await saveDb(d);
          savedItem = { ...newItem, duplicate: false };
          // Forward the freshly archived file to the link bot ONCE (via the
          // existing instant-get machinery) so the card gets a /dl/ link.
          if (instantGet) {
            const iw = await ensureInstantGet(d, newItem.id, newItem.url || cardTg, newItem.title, newItem.thumbnailUrl, newItem.telegramUrl);
            if (iw) instantRequest = iw;
          }
        }
        if (savedItem) {
          r.savedItemId = savedItem.id;
          r.cardSaved = true;
          await saveDb(d);
        }
      }
...
// secretary_poller forwards the file to the link generator exactly once. Avoids
// duplicate pending requests for the same card. `movieUrl` is used by the bot
// to resolve the file for forwarding.
async function ensureInstantGet(d, movieId, movieUrl, movieTitle, thumbnailUrl, telegramUrl) {
  d.instantRequests = d.instantRequests || [];
  const existing = d.instantRequests.find((r) => r.movieId === movieId && r.status === 'pending');
  if (existing) return existing;
  const request = {
    id: makeId(),
    movieId,
    movieTitle: String(movieTitle || 'Untitled').trim(),
    movieUrl: String(movieUrl || '').trim(),
    thumbnailUrl: String(thumbnailUrl || '').trim(),
    telegramUrl: String(telegramUrl || '').trim(),
    status: 'pending',
    resultUrl: null,
