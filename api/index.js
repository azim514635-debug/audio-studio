...
function resolveServiceAccount() {
  ...
  const rootDir = path.join(__dirname, '..'); // project root (api/ lives under it)
...
/* ------------------------------------------------------------------ */
/* Vercel: signed direct-to-Cloudinary uploads                         */
/* ------------------------------------------------------------------ */
/* On Vercel serverless the request body limit (4.5MB on Hobby) and lack
   of reliable multipart streaming make proxying file uploads through the
   server impractical. Instead the browser uploads the file straight to
...
/* Vercel serverless: do not bind a port. Only the local/dev flow starts the
   HTTP server. On Vercel, the exported `app` is used as the request handler. */
if (process.env.VERCEL !== '1' && !process.env.NEXT_RUNTIME) {
